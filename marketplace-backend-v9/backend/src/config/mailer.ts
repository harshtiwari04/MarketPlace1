...
/**
 * SendGrid Web API client. Unlike SMTP, there's no persistent connection/pool to manage — each
 * send is a single authenticated HTTPS request, so there's nothing to open at boot or drain at
 * shutdown.
 *
 * `null` when SENDGRID_API_KEY isn't configured (dev/test) — email.service logs instead of sending.
 */
if (env.EMAIL_ENABLED) {
  sgMail.setApiKey(env.SENDGRID_API_KEY!);
}

export const mailer = env.EMAIL_ENABLED ? sgMail : null;

/**
 * Best-effort startup probe — hits SendGrid's account endpoint so a bad/revoked API key is caught
 * at deploy time, not when the first customer never receives their code. Never throws: a SendGrid
 * hiccup at boot must not take the whole API down.
 */
export const verifyMailer = async (): Promise<void> => {
  if (!mailer) {
    logger.warn('SENDGRID_API_KEY not configured — emails will be logged, not sent (fine for dev/test only)');
    return;
  }
  try {
    ...
    logger.error('SendGrid verification failed — verification/reset emails will NOT be delivered until fixed', {
      err: err instanceof Error ? err.message : String(err),
    });
  }
};
...
export const closeMailer = (): void => {
  // No persistent connection to close — SendGrid sends are one-shot HTTPS requests. Kept as a
  // no-op so callers (server shutdown hooks) don't need to change.
};
